...
public final class FilebenchBenchmarkDriver extends BenchmarkDriver<IndependentVariablesOfFilebench, DependentVariables>
{
	...
	private boolean observeBenchmark(RemoteProcess benchProcess, RemoteConnection conn, String command, int timeout) throws InterruptedException, CheckedBenchmarkException
	{
		int watchDogCount = 0;
...
//						currentProcess = benchProcess;
						watchDogCount = 0;
						
						// Filebench restarted, reset start time
						startTime = System.currentTimeMillis(); 
						
						throw new CheckedBenchmarkException("Filebench malfunction.");
					}
					else
						++watchDogCount;
				}
	 ...
	 */
	@Override
	protected void prepareExperiment(IndependentVariablesOfSut sutVars, IndependentVariablesOfFilebench benchVars) throws BenchmarkException, CheckedBenchmarkException
	{
		...
		currentProcess = filebenchWarmup;
		String stdOut;

		try
		{
			RemoteConnection conn = ConnectionCollector.getAdditionalConnection(sut);
...
//			conn.open(); // remember to close before returning / proceeding
			...
			boolean timeoutOccured = observeBenchmark(filebenchWarmup, conn, command, benchVars.getWarmupTime()*2 + timeoutPrepare);
//			conn.close();
			
			if (timeoutOccured)
			{
				LOGGER.debug("Warm-up was taking too long, restarting...");
				prepareExperiment(sutVars, benchVars);
			}
			
			if ((System.currentTimeMillis() - startTime) < 2000)
			{
				prepareExperiment(sutVars, benchVars);
				LOGGER.debug("Warm-up was too short, restarting...");
			}
...
//			String targetDir = this.targetDir + fs.getDirectory().getAbsolutePath();
			String targetDir = fs.getDirectory().getPath();
	 ...
	 */
	@Override
	public DependentVariables startExperiment(int repeatNo) throws BenchmarkException, CheckedBenchmarkException
	{
...
//			conn.open(); // remember to close before returning / proceeding
			long startTime = System.currentTimeMillis();
			boolean timeoutOccured = observeBenchmark(filebenchBench, conn, command, runTime * 2);
//			conn.close();
			
			if (timeoutOccured)
			{
				LOGGER.error("Benchmark run was taking too long, Filebench might have been frozen");
				return null;
			}
			
			if ((System.currentTimeMillis() - startTime) / 1000 < runTime)
			{
				LOGGER.error("Benchmark run was too short. Restarting...");
				return startExperiment(repeatNo);
			}
			
			//conn = new SSHRemoteConnection(sut);
			//conn.open();
			//checkAndCloseFilebench(conn);
			//conn.close();
	...
	/*
	 * Deletes the two configfiles from the SUT.
	 */
	@Override
